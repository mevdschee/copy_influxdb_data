...
echo Progress:
influx $INFLUX_ARGS -execute "CREATE DATABASE $DST_DB WITH DURATION inf rentention 1 NAME $DST_RP"
for MEASUREMENT in $(influx $INFLUX_ARGS -database $SRC_DB -execute "show measurements" -format csv | tail -n +2); do
    echo -n "  - $MEASUREMENT: "
